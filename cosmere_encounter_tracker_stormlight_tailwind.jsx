...
import { 
  Plus, Save, Trash2, RefreshCcw, Edit3, ChevronRight, Settings,
  Swords, Shield, Zap, Moon, Brain, Activity, AlertTriangle, Pause, EyeOff
} from "lucide-react";

/**
 * Cosmere Encounter Tracker — Stormlight Theme
 * Single-file React component (Tailwind + shadcn/ui) with:
 * - Encounter builder (players & enemies)
 * - Round tracker: Fast Players → Fast Enemies → Slow Players → Slow Enemies (players always first)
 * - Per-entity: default Fast/Slow, type (player/enemy), status effects (conditions + Investiture), reaction usage, unconscious
 * - Drag & drop between Fast/Slow lanes OR one-click toggle
 * - Autosave & restore (localStorage by default), switchable storage in Admin
 * - High-contrast & dyslexia-friendly toggles
 * - Stormy blue theme with hints of green & purple
 */
...
const glow = "shadow-[0_0_30px_rgba(56,189,248,0.2)]"; // cyan glow
const stormText = "text-slate-100";
const accentBlue = "focus:ring-2 focus:ring-cyan-400/80";
const hintGreen = "bg-emerald-700/30";
const hintPurple = "bg-purple-700/20";

// Accessibility: dyslexia-friendly font via CSS var fallback
const dyslexiaClass = "[font-family:Inter,Atkinson_Hyperlegible,system-ui,sans-serif]";

// ---------- Main Component ----------
export default function CosmereEncounterTracker() {
  const [entities, setEntities] = useState([]);
  const [round, setRound] = useState(1);
  const [phaseIndex, setPhaseIndex] = useState(0);
  const [storageKind, setStorageKind] = useState(/** @type {"local"|"session"} */("local"));
  ...
  useEffect(() => {
    try {
      ...
      store.setItem(
        STORAGE_KEYS.APP,
        JSON.stringify({ entities, round, phaseIndex })
      );
    } catch {}
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        ...
        JSON.stringify({ storageKind, highContrast, dyslexiaMode })
      );
    } catch {}
  ...
  function addEntity(partial = {}) {
    ...
    setEntities(prev => [
      ...prev,
      {
        ...
        hp: 10,
        maxHp: 10,
        speed: "fast", // fast | slow (default configurable per-entity)
        defaultSpeed: "fast",
        statuses: [],
        statusInput: "",
        reactionUsed: false,
  ...
  function resetReactions() {
    setEntities(prev => prev.map(e => ({ ...e, reactionUsed: false })));
  }
  ...
  function nextPhase() {
    ...
    if (next === 0) {
      // new round
      setRound(r => r + 1);
      // reset reactions at the start of the round
      resetReactions();
      // snap each entity to its default speed at round start
  ...
  function onDrop(e, lane) {
    ...
    // Lane is like "FAST_PLAYERS", "SLOW_ENEMIES"
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <section {...laneProps("FAST_PLAYERS")} className={laneBoxStyle}>
                      <div className={laneHeaderStyle}><Swords className="inline size-3 mr-1"/>Players first</div>
                      <div className="mt-2 grid gap-2">
                        {fastPlayers.map(e => (
                          <EntityCard key={e.id} entity={e} onUpdate={updateEntity} onRemove={removeEntity}/>
                        ))}
                        {fastPlayers.length === 0 && <EmptyLaneHint text="Drag or toggle a player here" />}
                      </div>
                    </section>
                    <section {...laneProps("FAST_ENEMIES")} className={laneBoxStyle}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <section {...laneProps("SLOW_PLAYERS")} className={laneBoxStyle}>
                    ...
                    <section {...laneProps("SLOW_ENEMIES")} className={laneBoxStyle}>
                      <div className={laneHeaderStyle}><Shield className="inline size-3 mr-1"/>Enemies</div>
                      <div className="mt-2 grid gap-2">
                        {slowEnemies.map(e => (
                          <EntityCard key={e.id} entity={e} onUpdate={updateEntity} onRemove={removeEntity}/>
              ...
              <Card className={`bg-slate-950/60 ${stormBorders}`}>
                <CardContent className="p-3">
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {entities.map(e => (
                      <RosterEditor key={e.id} entity={e} onUpdate={updateEntity} onRemove={removeEntity} />
                    ))}
                  </div>
                  {entities.length === 0 && (
                    <div className="text-sm text-slate-400 p-4">No entities yet. Use buttons below to add some.</div>
                  )}
...
// ---------- Components ----------
function EmptyLaneHint({ text }) {
  return (
    <div className="text-xs text-slate-400/90 italic px-2 py-3 rounded-xl bg-slate-800/40 border border-slate-700/40">
      {text}
    </div>
  );
}

function RosterEditor({ entity, onUpdate, onRemove }) {
  return (
    <Card className="bg-slate-900/70 border-slate-700/70">
      <CardContent className="pt-4">
        ...
        <div className="mt-3">
          ...
          <textarea
            className="w-full mt-1 rounded-xl bg-slate-950/60 p-2 border border-slate-700/60"
            rows={3}
            value={entity.notes}
            onChange={(e) => onUpdate(entity.id, { notes: e.target.value })}
          />
        </div>
      </CardContent>
    </Card>
...
function EntityCard({ entity, onUpdate, onRemove }) {
  ...
  return (
    <div
      ...
      className={`${unconscious ? "opacity-50 grayscale" : ""} ${investitureColor} ${stormBorders} rounded-2xl p-3 hover:outline ${glow} outline-cyan-400/40`}
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="outline" className="border-slate-500/60 text-slate-200/90">{entity.speed.toUpperCase()}</Badge>
        </div>
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-sm">
        <div className="col-span-2 flex items-center gap-2">
          ...
          <Input
            type="number"
            className="bg-slate-950/60 h-8"
            value={entity.hp}
            onChange={(e) => onUpdate(entity.id, { hp: Number(e.target.value) })}
            aria-label="HP"
          />
          ...
          <Input
            ...
            className="bg-slate-950/60 h-8"
            value={entity.maxHp}
            onChange={(e) => onUpdate(entity.id, { maxHp: Number(e.target.value) })}
            aria-label="Max HP"
          />
        </div>
      ...
      <div className="mt-3">
        ...
        <div className="mt-2 flex gap-2">
          <Input
            placeholder="Custom effect (e.g., Stormlight: 2 charges)"
            value={entity.statusInput}
            onChange={(e) => onUpdate(entity.id, { statusInput: e.target.value })}
            className="bg-slate-950/60 h-9"
          />
          <Button
            onClick={() => {
              if (!entity.statusInput.trim()) return;
              onUpdate(entity.id, {
                statuses: [...entity.statuses, entity.statusInput.trim()],
                statusInput: "",
              });
            }}
            className="bg-emerald-700/60 hover:bg-emerald-600/60"
          >Add</Button>
        </div>
      </div>
    </div>
...
function AdminPanel({ storageKind, setStorageKind, highContrast, setHighContrast, dyslexiaMode, setDyslexiaMode }) {
  return (
    <Dialog>
